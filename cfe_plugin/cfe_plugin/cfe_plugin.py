...
class FSWPlugin(FSWPluginInterface):
    ...
    def __init__(self, node):
        ...
        self._telemetry_port = self._node.get_parameter('plugin_params.telemetryPort').get_parameter_value().integer_value
        ...
        # self.cfs_root = self._node.get_parameter('/home/swhart/code/cFS').get_parameter_value().string_value
        ...
        #     if deffile == "null":
        #         continue
        #     self._recv_map[key] = CFSTelemReciever(self.telem_pages_info, key, self.telem_pages_info.getTelemAppid(i), f"{self.ROOTDIR}/tlmGUI/", deffile)
        #     topicName = Path(deffile).stem.replace("-", "_")
        #     t = TelemInfo(key, msgType, topicName)
        #     self._telem_info.append(t)

        self._telem_receiver = TelemReceiver(self._node, self._msg_pkg, self._telemetry_port, self._telemetry_dict, self._juicer_interface.getMsg_list())
        # self.broad_map = {}
